...
If you want to build and install the latest version of `bat` via its
[crate](https://crates.io/crates/bat/versions), you need Rust 1.45 or higher.
You can then use `cargo` to build and install `bat`:

```bash
cargo install bat
```

Note that only the `bat` binary will be installed in this way. Additional files
like the man page or shell completion files will not be installed.

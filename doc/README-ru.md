...
Даже в `zsh` лучше использовать функцию-обёртку выше, а не глобальные алиасы
для `-h` и `--help`. Глобальные алиасы раскрываются внутри подключаемых
скриптов и условий вроде `[[ -h /path ]]`, из-за чего могут возникать ошибки
разбора.
